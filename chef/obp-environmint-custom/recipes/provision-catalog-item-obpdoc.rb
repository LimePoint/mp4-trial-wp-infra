...
password = PasswordVault.get_password(password_vault_name, 'obpdoc', 'keystorepass')
PasswordVault.put_password(password_vault_name, code, 'keystorepass', password)

if environment_name =~ /^prd/ or environment_name =~ /^svp/
	#Load password of env specific prefix password into the generic prefix(duplicate).
	password = PasswordVault.get_password(password_vault_name, 'obpdoc', "#{environment_name.upcase}OBPDOC")
	PasswordVault.put_password(password_vault_name, code, 'OBPDOC', password)
end

#### End asset specific code      ####

### Add this to calculate Coherence Parameters ###
mintpress_resource "coherence-config" do
	asset code
	type "CoherenceListen"
	value "%LISTENNAME%-prv#{asset_vars['dns_domain_name']}"
end
...
consoles = [
	...
	{
		"itemCode" => _item_code,
		"name" => "documaker_admin_console",
		"description" => "Documaker Administrator",
		"url" => asset_vars['frontend']['protocol'] + '://' + asset_vars['frontend']['otd_vip_name'] + ":" + asset_vars['frontend']['dmkr_port'] + "/DocumakerAdministrator"
	},
	{
		"itemCode" => _item_code,
		"name" => "documaker_interactive_console",
		"description" => "Oracle Documaker Interactive: Correspondence",
		"url" => asset_vars['frontend']['protocol'] + '://' + asset_vars['frontend']['otd_vip_name'] + ":" + asset_vars['frontend']['idm_port'] + "/DocumakerCorrespondence"
	}
]
