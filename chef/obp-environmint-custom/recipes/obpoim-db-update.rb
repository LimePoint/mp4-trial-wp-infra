...
template "Processing obpoim-manage-pwd.sql" do
	source "sql/obpoim-manage-pwd.sql.erb"
	path "#{base_dir}/obpoim-manage-pwd.sql"
	variables(
		:username => schema_name,
		:password => Mint::AesEncryption.decrypt(PasswordVault.secret("databag://#{password_vault_name}/#{asset_code}/OBPOIM-MDS").value)
	)
	mode '0644'
	user 'oracle'
	group 'oinstall'
...
oracle_sql "#{base_dir}/obpoim-manage-pwd.sql" do
	...
	as_sysdba false
	sql_file "#{base_dir}/obpoim-manage-pwd.sql"
	action :run
	user 'oracle'
	group 'oinstall'
end
