...
template "Processing RemoveBIResource.py" do
	source "fmw/wlst/RemoveBIResource.py.erb"
	path "#{base_dir}/RemoveBIResource.py"
	mode '0644'
	user 'oracle'
	group 'oinstall'
end	

bash 'Executing RemoveBIResource.py' do
  code <<-EOH
    /oracle/app/binaries/#{asset_code}/fmw/oracle_common/common/bin/wlst.sh #{base_dir}/RemoveBIResource.py weblogic #{weblogicAdminPassword} #{weblogicAdminUrl}
    EOH
end