...
template 'Template LDAP Adapters Condfig Script' do
  source "fmw/wlst/updateLDAPAdaptersConfig.py.erb"
  path "/tmp/updateLDAPAdaptersConfig.py"
  mode '0700'
end

bash 'Updating LDAP Adapter Config' do
  code <<-EOH
    /oracle/app/binaries/#{asset_code}/fmw/oracle_common/common/bin/wlst.sh /tmp/updateLDAPAdaptersConfig.py weblogic #{weblogicAdminPassword} #{weblogicAdminUrl} #{asset_code}
    EOH
end