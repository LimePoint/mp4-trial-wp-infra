...
template 'ipm_app_import.py.erb' do
  ...
  variables(
      :app_definition_xml => "#{as_domain_home}/ipm_dcms_app_definition.xml"
  )
  mode '0700'
  end

#IPM APP Definitions File
template 'ipm_dcms_app_definition.xml.erb' do
  source "fmw/obpipm/ipm_dcms_app_definition.xml.erb"
  path "#{as_domain_home}/ipm_dcms_app_definition.xml"
  mode '0700'
  end

#Update APP
bash "Configuring IPM DCMS app" do
  code <<-EOH
    /oracle/app/binaries/#{asset_code}/fmw/wcc_11.1/common/bin/wlst.sh #{tmp_dir}/ipm_app_import.py obpipm_admin #{obpipm_adminPassword} #{asset_vars['ipm_server']['listen_address']} DCMSDocuments Imported
    EOH
  only_if "cat /oracle/app/runtime/obpipm/#{environment_name}/ucm/cs/data/ipmsys/apps/*.hda | grep 'ApplicationName=DCMSDocuments'"
end

#Update dcms_metadata_properties for the new FS Option
...
ruby_block "Modify config.cfg file #{config_file}" do
    block do
      ...
      file.search_file_replace_line(/IPM_FILE_STORAGE_DB=CSHIPMDB/, "IPM_FILE_STORAGE_DB=DCMSDOCSFS")
      file.write_file
    end
