...
**Shutdown the source OIM 11g Instances** (can use RunTime shutdown plan from original 11g obpoim)

* Update env data bag for target asset obpoim with "release_version": "1.7.0" include new database details if new DB is to be created
...
OIM / CIM DirectDB config Update
1)	Login to Enterprise Manager by using the following URL:
2)	https://obpwstg1oim-adm.radtest.wbctestau.westpac.com.au:18101/em
...
```
cd /oracle/app/binaries/oim12c/fmw/idm/server/upgrade
...
cd /oracle/app/runtime/obpoim/domains; grep -lIR "obprsvp2oim" | xargs sed -i "s|obprsvp2oim|obprsvp3oim|g";
cd /oracle/app/runtime/obpoim/domains; grep -lIR '/oracle/app/runtime/svp2r/certs/'| xargs sed -i 's|/oracle/app/runtime/svp2r/certs/|/oracle/app/runtime/svp3r/certs/|g';
cd /oracle/app/runtime/obpoim/domains; grep -lIR 'svp2' | xargs sed -i 's|svp2|svp3|g';

cd /oracle/app/binaries/obpoim; grep -lIR 'obprsvp2oim' | xargs sed -i 's|obprsvp2oim|obprsvp3oim|g';
cd /oracle/app/binaries/obpoim/fmw; grep -lIR 'svp2' | xargs sed -i 's|svp2|svp3|g';
mkdir -p /oracle/app/logs/obpoim/obprsvp3oim01/; mkdir -p /oracle/app/logs/obpoim/obprsvp3oim02/; 
...
```
* (Only on first node ex: obprsvp3oim01)
...
# Part 2 - Run Upgrade Readiness (Only on first node ex: obprsvp3oim01)
```
...
# Part 3 - Upgrade Database Schemas (Only on first node ex: obprsvp3oim01)
```
...
# Part 4 - Reconfigure Domain (Only on first node ex: obprsvp3oim01)
```
...
# Part 4a - Upgrade Domain Components (Only on first node ex: obprsvp3oim01)
```
...
# Part 5 - Execute post upgrade changes (On Both the nodes ex: obprsvp3oim01 & obprsvp3oim02)
```
...
```
rm -rf /oracle/app/runtime/obpoim/domains/obpoim_domain/servers/soa_server* /oracle/app/runtime/obpoim/domains/obpoim_domain/servers/oim_server* /oracle/app/runtime/obpoim/domains/obpoim_domain/servers/bip_server*;
```
...
export NODEMGR_HOME=/oracle/app/binaries/oim12c/fmw/wlserver/common/nodemanager;
cd /oracle/app/binaries/oim12c/fmw/wlserver/server/bin;
nohup ./startNodeManager.sh & 


* Start Admin Server Manually
cd /oracle/app/runtime/obpoim/domains/obpoim_domain/;
nohup ./startWebLogic.sh &

* Once Admin Server is up we need to update JDBC to XA Driver & Remove the RemoveBIResource (Only on first node ex: obprsvp3oim01)
...
* The below receipe is required only Multinode environments where we used the above work arround
...
* Update the obpoim component in the environment data bag file to use the 12c template by defining the appropriate release_version
"release_version": "1.7.0",

* Do **Upload Only** in MintPress Console for the OBPOIM instance being upgraded, to generate a new JSON with properties relevant for 12c, and open the RunTime project created
...
/Domain_obpoim_domain/obpoim_domain > System MBean Browser
oracle.iam > Server:oim_server1 > Application:OIM > XMLConfig > Config > XMLConfig.DiscoveryConfig:Discovery

Update the OimFrontEndURL Attribte value to right address ex: https://obpdev3oim.nonprod.mte.westpac.com.au 

* Login into Sysadmin Console and update It resources ( OID Server , DBAT_1_4 ) with correct details ( DBName , OID Name , Passwords etc)
...
Drop the OBPOIM_BIPLATFORM user from the database
```
Drop user OBPOPIM_PLATFORM cascade;
Drop tablespace OBPOIM_BIPLATFORM including contents and datafiles;
```

# Part 11 - Cleanup
Remove the backup of the 11g binaries 
```
rm -rf/oracle/app/binaries/obpoim_11g
```
